...
def noise_like(shape, device, repeat=False):
    repeat_noise = lambda: torch.randint((1, *shape[1:]), device=device).repeat(
        shape[0], *((1,) * (len(shape) - 1))
    )
    noise = lambda: torch.randint(shape, device=device)
...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        Diffuse the initial input.
            ...
            :return: [B, T, *]
        """
        noise = default(noise, lambda: torch.randint_like(x_start))
        return (
    ...
    def q_sample_target(self, y_target, t, noise=None):
        ...
        Diffuse the target.
            ...
            :return: (tensor) [B1, T1, *]
        """
        noise = default(noise, lambda: torch.randint_like(y_target))
    ...
    def p_losses(self, x_start, y_target, t,  noise=None, noise1=None):
        ...
        noise = default(noise, lambda: torch.randint_like(x_start, 0, 1))
        noise1 = default(noise1, lambda: torch.randint_like(y_target, 0, 1))
