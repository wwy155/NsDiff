...
class SyncBatchNormSwish(nn.BatchNorm2d):
    ...
    def forward(self, input):
        ...
        # Swish Activation: Swish = x * sigmoid(x)
...
class SwishFN(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        # Unpack the saved tensor correctly
...
#             self.log_weight_norm = torch.tensor(torch.log(init + 1e-2), requires_grad= False)
...
class FactorizedReduce(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(conv4.shape)
        out = torch.concat([conv1, conv2, conv3, conv4], 1)
        return out
...
class InvertedResidual(nn.Module):
    def __init__(self, Cin, Cout, stride, ex, dil, k, g):
        ...
        layers0 = [nn.Upsample(scale_factor=2, mode='nearest')] if self.upsample else []
