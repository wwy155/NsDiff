...
class CSDI_base(nn.Module):
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        # repeat for parallel accelarated generation
        def repeat(tensor, dim=0):
            return tensor.repeat_interleave(repeats=n_samples, dim=dim)

        observed_data = repeat(observed_data)
        cond_mask = repeat(cond_mask)
        side_info = repeat(side_info)
        ...
        all_samples = torch.randn_like(observed_data)

        for t in range(self.num_steps - 1, -1, -1):
            if self.is_unconditional == True:
                diff_input = cond_mask * noisy_cond_history[t] + (1.0 - cond_mask) * all_samples
                diff_input = diff_input.unsqueeze(1)  # (B,1,K,L)
            else:
                cond_obs = (cond_mask * observed_data).unsqueeze(1)
                noisy_target = ((1 - cond_mask) * all_samples).unsqueeze(1)
                diff_input = torch.cat([cond_obs, noisy_target], dim=1)  # (B,2,K,L)
            predicted = self.diffmodel(diff_input, side_info, torch.tensor([t]).to(self.device))

            coeff1 = 1 / self.alpha_hat[t] ** 0.5
            coeff2 = (1 - self.alpha_hat[t]) / (1 - self.alpha[t]) ** 0.5
            all_samples = coeff1 * (all_samples - coeff2 * predicted)

            if t > 0:
                noise = torch.randn_like(all_samples)
                sigma = (
                    (1.0 - self.alpha[t - 1]) / (1.0 - self.alpha[t]) * self.beta[t]
                ) ** 0.5
                all_samples += sigma * noise
        
        imputed_samples = all_samples.reshape(B, n_samples, K, L )
    ...
    def forward(self, batch, is_train=1):
        ...
        if is_train:
            noise, predicted = self.train_forward(observed_data, cond_mask, observed_mask, side_info)
            return noise, predicted
        else:
            NotImplementedError("we do not caculate loss during validation phase!!!")
        
        
    def train_forward(self, observed_data, cond_mask, observed_mask, side_info):
        B, K, L = observed_data.shape
        if is_train != 1:  # for validation
            t = (torch.ones(B) * set_t).long().to(self.device)
        else:
...
class CSDI_Forecasting(CSDI_base):
    ...
    def get_side_info(self, observed_tp, cond_mask,feature_id=None):
        ...
        if self.target_dim == self.target_dim_base:
            ...
            feature_embed = self.embed_layer(feature_id).unsqueeze(1).expand(-1,L,-1,-1)  # (B, L, K, 1)
        side_info = torch.cat([time_embed, feature_embed], dim=-1)  # (B,L,K,*)
        side_info = side_info.permute(0, 3, 2, 1)  # (B,*,K,L)
